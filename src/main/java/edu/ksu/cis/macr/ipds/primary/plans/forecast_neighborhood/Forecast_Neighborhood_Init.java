 ...
 */
public class Forecast_Neighborhood_Init implements IPlanState<Forecast_Neighborhood_Plan> {
  private static final Forecast_Neighborhood_Init instance = new Forecast_Neighborhood_Init();

  private static final Logger LOG = LoggerFactory.getLogger(Forecast_Neighborhood_Init.class);

  private Forecast_Neighborhood_Init() {
  }

  /**
   @return the singleton instance
   */
  public static Forecast_Neighborhood_Init Instance() {
    return instance;
  }
  ...
  public synchronized void Execute(final IExecutablePlan plan, final IExecutor ec, final InstanceGoal<?> ig) {
    ...
    if ((RunManager.isStopped())) {
      plan.getStateMachine().changeState(Forecast_Neighborhood_Stop.Instance(), ec, ig);
    }
  }
