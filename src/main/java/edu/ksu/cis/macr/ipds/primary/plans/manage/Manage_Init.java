 ...
 */
public enum Manage_Init implements IPlanState<Manage_Plan> {
  INSTANCE;
  private static final Logger LOG = LoggerFactory.getLogger(Manage_Init.class);
  ...
  public synchronized void Execute(final IExecutablePlan plan, final IExecutor ec, final InstanceGoal<?> ig) {
    ...
    Objects.requireNonNull(ec.getCapability(ManageCapability.class), "Role needs ManageCapability");
    ...
    if (RunManager.isInitiallyConnected()) {
        ...
        if (ec.getCapability(ParticipateCapability.class) != null) {
            ...
            plan.getStateMachine().changeState(Manage.INSTANCE, ec, ig);
        }
    }
