...
import edu.ksu.cis.macr.ipds.grid.GridOrganizationFactory;
import edu.ksu.cis.macr.ipds.grid.messages.PowerPerformative;
...
/**
 The {@code GridAdminCapability} provides the functionality necessary to create a new organization.
 */
public class GridAdminCapability extends AbstractOrganizationCapability {
    private static final Logger LOG = LoggerFactory.getLogger(GridAdminCapability.class);
    private static final boolean debug = false;
    private static final IMessagingFocus messagingFocus = GridMessagingFocus.GRID_PARTICIPATE;
    private static Channel channel = PowerPerformative.MessagingManager.getChannel(messagingFocus);
    ...
    /**
     Construct a new {@code GridAdminCapability} instance.
     ...
     */
    public GridAdminCapability(final IPersona owner, final IOrganization org) {
        super(GridAdminCapability.class, owner, org);
    ...
    public void init(InstanceGoal<?> ig) {
        ...
        if (this.childConnections == null) {
            LOG.error("Super holon trying to initialize an external organization with no participants. Guidelines={}", params);
            System.exit(-59);
        } else {
            if (debug) LOG.info("Starting initialization of new organization by {}. ", this.getOwner());
            this.setChildConnections(childConnections);
            if (debug)
                LOG.debug("{} authorized connections to sub holons.", childConnections.getListConnectionGuidelines().size());
    ...
    public IOrganization createOrganization(InstanceGoal<?> ig) {
        ...
        GridHolonicLevel level = GridHolonicLevel.getOrganizationType(orgName);
        ...
        if (debug) LOG.debug("The organization level is {}", level);
        ...
        this.myorg = new GridOrganizationFactory().createBaseAffiliateOrganization(folder.getAbsolutePath(),
                OrganizationFocus.External, orgParams,
                this.orgModelFolder, goalFilePath, roleFilePath, RunManager.getTopGoal());
        LOG.info("Grid org initialized with agents and top goal guidelines. Standard models in {} subfolder.", myorg.getOrgModelFolder());
        return this.myorg;
    }
            ...
            this.orgModelFolder = Objects
                    .requireNonNull((IConnections) params
                            .getValue(StringIdentifier
                                    .getIdentifier("childConnections"))).getListConnectionGuidelines().get(0).getOrgModelFolder();
            if (this.orgModelFolder.isEmpty()) {
                LOG.error("ERROR: could not get path the folder with this orgs standard models. ");
                System.exit(-22);
            }
    ...
    private boolean areThereEnoughAgentsForInitialAssignments() {
        if (debug) LOG.debug("In GridAdminCapability of {} " +
    ...
    private boolean remoteSendAssignmentMessage(edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
        ...
        final String fullQueueName = GridMessagingManager.getFullQueueName(receiver, GridMessagingManager.getQueueFocus(messagingFocus));
        final String routingKey = fullQueueName;
        GridMessagingManager.declareAndBindConsumerQueue(messagingFocus, receiver);
        try {
    ...
    public Set<Agent<?>> getLocalRegisteredProsumers(Set<Agent<?>> allAgents) {
        ...
        if (debug) LOG.debug("Number of all agents found in the GridAdminCapability is {}",
                allAgents.size());

        final HashSet<Agent<?>> prosumers = new HashSet<>();
        Iterator<Agent<?>> it = allAgents.iterator();

        final Class<?> capabilityClass = AssessReactivePowerQualityCapability
                .class;
    ...
    public synchronized void initializeGuidelines(InstanceGoal<?> instanceGoal) {
        ...
        this.setChildConnections(childConnections);
        if (debug)
            LOG.debug("There are {} authorized connections to sub holon agents.", childConnections.getListConnectionGuidelines().size());
    }
