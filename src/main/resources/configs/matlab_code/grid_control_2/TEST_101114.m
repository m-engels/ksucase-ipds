...
% Qrs= reactive power responses from feeder holons
...
[childTarget,parentResponse]=OPF_G(ieee_Sub,iter,pT,cR);
...
for i=1:Num_bus % for all feeder holons
    ...
    elseif (i~=3)&&(i~=8)&&(i~=25)&&(i~=28)&&(i~=31)&&(i~=37)
        ...
        for JJ=1:length(Net.SubNode(i).FeederNode)
            ...
            OPF_N(ieee_N,Ptn(JJ),Qtn(JJ),V_F(JJ+1),Teta_F(JJ+1),Phase);
