function [Voltage,Angle,Qinjection,Pr,Qr,deltaP_f2n,deltaQ_f2n,Max_Feeder]=...
    OPF_N(ieee37_bus_data1,PPt,QQt,VVoltage,AAngle,Phase)

global Max_Neighbor_Iter 
% i=19
% LABC=[7;8]
% GABC=14
GABC=Phase+13;
...
% [x,fval,exitflag] = fmincon(@(x)LossObjectiveFunctionSub(x,Num_gen_Sub,Pflow_f2n,Qflow_f2n),ones(1,2*Num_bus_Sub+Num_gen_Sub+1),[],[],[],[],[],[],@(x)LossConstraints1(x,Num_bus_Sub,Num_gen_Sub,Bus_ref_Sub,Limit_Slack_Sub,Gen_bus_Sub,Q_Lim_Sub,Q_dim_Sub,P_dim_Sub,P_gen_Sub,Ybus_abs,Ybus_ang,v,V_angle),options);

%[x,fval,exitflag,output] = fmincon(@(x)LossObjectiveFunctionSub(x,Num_gen_Sub,Pflow_f2n,Qflow_f2n,Net,i,GABC),ones(1,2*Num_bus_Sub+Num_gen_Sub+1),[],[],[],[],[],[],@(x)LossConstraints1(x,Num_bus_Sub,Num_gen_Sub,Bus_ref_Sub,Limit_Slack_Sub,Gen_bus_Sub,Q_Lim_Sub,Q_dim_Sub,P_dim_Sub,P_gen_Sub,Ybus_abs,Ybus_ang,v,V_angle),options);
[x,fval,exitflag,output] = fmincon(@(x)Obj_N(x,Num_bus_Sub,Pflow_f2n,Qflow_f2n),ones(1,2*Num_bus_Sub+Num_gen_Sub+1),[],[],[],[],[],[],@(x)LossConstraints1(x,Num_bus_Sub,Num_gen_Sub,Bus_ref_Sub,Limit_Slack_Sub,Gen_bus_Sub,Q_Lim_Sub,Q_dim_Sub,P_dim_Sub,P_gen_Sub,Ybus_abs,Ybus_ang,v,V_angle),options);
...
Max_Feeder=max(Max_Neighbor_Iter,output.iterations);
...
%Qt=(Q_dim_Sub(2:end,:))*baseMVA*1000-Qinjection;
deltaP_f2n=abs(Pr-PPt*baseMVA*1000);
deltaQ_f2n=abs(Qr-QQt*baseMVA*1000);
% end
